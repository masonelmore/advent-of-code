...
def main():
    ...
    parser.add_argument('-p', '--puzzle', dest='puzzle', type=str, action=PaddingAction)
    args = parser.parse_args()

    solve = get_solve_func(args.year, args.day, args.puzzle)
    input_filename = f'{args.year}/d{args.day}_input.txt'
    input_data = load_input(input_filename)
    solution = solve(input_data)
    print(f'{args.year}/d{args.day}p{args.puzzle}: {solution}')
...
def get_solve_func(year, day, puzzle):
    filename = find_solution_file(year, day, puzzle)
    module = importlib.import_module(f'{year}.{filename}')
    return module.solve


def find_solution_file(year, day, puzzle):
    file_pattern = f'{year}/d{day}p{puzzle}_*'
